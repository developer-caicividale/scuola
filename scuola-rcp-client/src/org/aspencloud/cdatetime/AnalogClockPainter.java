...
import org.aspencloud.v.IControlPainter;
...
//	private int bezelWidth = 5;
	...
	public AnalogClockPainter(AnalogTimePicker picker) {
		this.picker = picker;
	}
	...
	public final void paintContent(VControl control, Event e) {
		...
		Calendar cal = picker.getCalendarInstance();
	...
	protected void paintFace(VPanel panel, Event e) {
		...
		Calendar tmpcal = picker.getCalendarInstance();
		tmpcal.set(1, 1, 1, 0, 0, 0);
		String pattern = picker.is24Hour ? "H" : "h";
		e.gc.setForeground(e.display.getSystemColor(SWT.COLOR_DARK_BLUE));
		...
		for(int i = 0; i < inc; i++) {
			...
			String str = picker.getFormattedDate(pattern, tmpcal.getTime());
