...
public class CDTSnippet01 {
	...
	public static void main(String[] args) {
		...
		int[] style = new int[] {
				CDT.BORDER | CDT.TIME_MEDIUM | CDT.DATE_LONG,
				CDT.BORDER | CDT.DATE_LONG,
				CDT.BORDER | CDT.TIME_MEDIUM
		};
		int[] style_ext = new int[] { CDT.SIMPLE, CDT.COMPACT | CDT.SIMPLE | CDT.SPINNER, CDT.DROP_DOWN };
			...
			for(int j = 0; j < style_ext.length; j++) {
				...
				cdts[ix].addSelectionListener(new SelectionAdapter() {
		        	...
		        	public void widgetSelected(SelectionEvent e) {
		        		...
		        		for (CDateTime dateTime : cdts) {
							if(dateTime != cdt) {
								dateTime.setSelectedDates(cdt.getSelectedDates());
							}
						}
		        	}
		        });
