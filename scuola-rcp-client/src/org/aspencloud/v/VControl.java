...
//	public static final int STATE_FOCUS = 1 << 3;
	...
	void attachListeners(boolean key, boolean set) {
		if(set) {
			if(key && this != VTracker.getFocusControl()) {
	...
	public void notifyListeners(int eventType, Event event) {
		if(listeners.containsKey(eventType)) {
			...
			for(Listener listener : listeners.get(eventType).toArray(new Listener[listeners.get(eventType).size()])) {
				listener.handleEvent(event);
			}
		}
	}
	...
	boolean setFocus(boolean focus) {
		if(!hasStyle(SWT.NO_FOCUS)) {
			...
			if(focus && VTracker.getFocusControl() != this) {
				getComposite().forceFocus();
				VTracker.instance().setFocusControl(this);
